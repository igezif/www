		...
		<div style = "vertical-align: top;">
			...
			<div style = "height: 1000px; overflow-y: scroll; width: 650px; border: 1px solid #808080;">
				<table class = "admin_table" id = "admin_slider_products"> 
					...
					<tr <?php if($product->id == $product_id) { ?>class = "admin_active_slider_product"<?php } ?>>
						<td><?=$i?></td>
						<td><?=$product->title?></td>
						<td><?=$product->meta_desc?></td>
						<td class = "admin_img"><img src = "<?=$product->img?>" /></td>
						<td class = "for_radio_td">
							<input name="product_id" type="radio" value="<?=$product->id?>" <?php if($product->id == $product_id) { ?>checked<?php } ?> />
						</td>
					</tr>
					<?php $i++; } ?>
				</table>
